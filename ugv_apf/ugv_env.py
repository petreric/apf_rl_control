...
from .simple_room import ROOM_CFG  # isort:skip
from .jetbot import JETBOT_CFG  # isort:skip

@configclass
class UgvSceneCfg(InteractiveSceneCfg):
    room = ROOM_CFG.replace(prim_path="/World/envs/env_.*/Room")
...
class UgvEnvCfg(DirectRLEnvCfg):
    ...
    scene: InteractiveSceneCfg = UgvSceneCfg(num_envs=400, env_spacing=11, replicate_physics=True)
    # robot
    robot: ArticulationCfg = JETBOT_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class UgvEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: UgvEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._body_id = self._robot.find_bodies("chassis")[0]
    ...
    def _apply_action(self):
        self._robot.set_joint_velocity_target(self.actions)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        rewards = {
            # "ang_vel_penalty": -ang_vel * 0.04 * self.step_dt,
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        died = torch.logical_or(self._robot.data.root_pos_w[:, 2] < -10.1, self._robot.data.root_pos_w[:, 2] > 2.0)
        ...
        dist_robot_to_goal = torch.norm(desired_pos_b, dim=-1)  # Distance in the body frame
        died = torch.logical_or(died, dist_robot_to_goal < 0.2)
